...
export function Simulazione() {
    ...
    const boolean = context.state.algoritmo

    const x = !boolean ? simulateInv(context.state, boolean) : algoritmoGenetico(70, context.state, boolean);
        ...
        <aside>
            ...
            <br/>
                ...
                <div className="container">
                    <ul >
                    {x.prezzi.map((el: number) => <li>{round(el)} €</li>)}
                    </ul>
                </div>
                <br/>
                <h3>Numero di quote comprate ogni volta: </h3>
                <ul>
                {x.quote.map((el: string) => <li>{el}</li>)}
                </ul><br/>
                <h3>Giorni d'acquisto:</h3>
                <div className="days-r">
                {x.giorni_comprasi.slice(0, 10).map((el: string) => <p>{el}</p>)}
                </div><br/>
                <h3>Giorni di guadagno:</h3>
                <div className="days">
                {x.giorni_vendesi.slice(0, 10).map((el:string) => <p>{el}</p>)}
                </div>
        </aside>
        </section>
    )
